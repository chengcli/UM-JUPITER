...
`manuscript/plot_precipitation_evaporation_timescales.py` makes the manuscript
comparison plot for fast-limit precipitation evaporation along the analytic dry
Jovian adiabat. It reads vapor-diffusion parameters and condensed-phase molar
volumes from the YAML file, assumes a maximally subsaturated environment
`chi_v = 0`, and plots the e-folding timescale implied by
`12 nu v_m chi_v^* / D_p^2` for H2O and NH3 precipitation particles.

```bash
python manuscript/plot_precipitation_evaporation_timescales.py
```

Defaults use particle diameters of 1, 2, 5, and 10 mm, pressure from 0.1 to
100 bar, and a 25 minute vertical reference line for sedimentation over one
30 km Jovian scale height at `20 m s^-1`. The curves use dashed H2O and solid
NH3 line styles. The horizontal axis is in seconds and spans up to `1e10 s`.
The PNG and CSV are written to `manuscript/`. Use `--diameters-mm`, `--pmin`,
`--pmax`, `--adiabat-kappa`, or `--config` to adjust the calculation.

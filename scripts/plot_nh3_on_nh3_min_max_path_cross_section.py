#!/usr/bin/env python3
"""Cache and plot NH3 on the periodic section through NH3 path extrema."""
...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/matplotlib-um-jupiter-nh3-path-section")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Cache and plot NH3 vapor/cloud on the NH3 path-extrema section."
    )
...
def main() -> None:
    ...
    if args.refresh_cache or not section.exists():
        build_cache(case_dir, section, "NH3", args.last)
        print(f"Wrote cache {section}")
    else:
        print(f"Using cache {section}")
    if args.cache_only:
        return

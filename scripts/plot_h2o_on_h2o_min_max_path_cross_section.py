...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=False,
        help="Overlay perpendicular-velocity contours. Default: disabled.",
    )
    parser.add_argument(
        "--show-vorticity",
        action=argparse.BooleanOptionalAction,
...
def output_path_for_overlay(
    ...
    return (
        output_dir
        / f"{case_name}_H2O_on_H2O_min_max_path_cross_section_{overlay}_last{last}.png"
    )


def overlay_name(show_perp_velocity: bool, show_vorticity: bool) -> str:
    if show_perp_velocity and show_vorticity:
        raise ValueError("Choose either wind or vorticity overlay, not both")

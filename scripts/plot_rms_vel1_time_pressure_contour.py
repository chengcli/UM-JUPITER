#!/usr/bin/env python3
"""Plot a time-pressure contour of horizontal RMS vertical velocity."""
...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/matplotlib-um-jupiter-rms-vel1-contour")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plot horizontal RMS vertical velocity as a filled contour versus "
            "time and log pressure for one CRM case."
        )
    )
    parser.add_argument(
        "--root",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Cache directory. Default: <output-dir>/rms_vel1_time_pressure_cache",
    )
    parser.add_argument(
        "--refresh-cache",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Centered running-average window in days. Default: no averaging",
    )
    parser.add_argument(
        "--vmax",
        type=float,
        default=4.0,
        help="Maximum contour value in m/s. Default: 4",
    )
    parser.add_argument(
        "--cmap",
        default="YlOrRd",
    ...
    if args.running_average is not None and args.running_average <= 0.0:
        parser.error("--running-average must be positive")
    if args.vmax <= 0.0:
        parser.error("--vmax must be positive")
    return args
...
def cache_path(
    ...
    return cache_dir / f"{case_name}_rms_vel1_time_pressure_{snapshot_suffix(first, last)}.npz"
...
def read_case_data(
    ...
    for file_info in files:
        ...
        rms_profiles.append(np.sqrt(np.nanmean(values * values, axis=(1, 2))))
        snapshots.append(file_info.snapshot)

    return (
        np.asarray(times, dtype=np.float64),
        np.stack(pressures),
        np.stack(rms_profiles),
        snapshots,
    )
...
def interpolate_to_pressure_grid(
    ...
    interpolated = []
    for pressure, rms in zip(pressure_bar, rms_profiles):
        keep = np.isfinite(pressure) & np.isfinite(rms) & (pressure > 0.0)
...
def filter_and_average_time(
    ...
    rms_grid = rms_grid[order]

    keep = np.isfinite(times_days)
    if max_time is not None:
        keep &= times_days <= max_time
    times_days = times_days[keep]
    rms_grid = rms_grid[keep]
    if times_days.size < 2:
        raise ValueError("Time selection must contain at least two snapshots")

    if running_average is None:
        return times_days, rms_grid

    half_window = running_average / 2.0
    averaged = np.empty_like(rms_grid)
    for index, time_day in enumerate(times_days):
        in_window = np.abs(times_days - time_day) <= half_window
        averaged[index] = np.nanmean(rms_grid[in_window], axis=0)
    return times_days, averaged
...
def plot_contour(
    ...
    contour_levels = np.arange(0.0, vmax + 0.125, 0.25)
    ...
    colorbar.set_label(r"RMS vertical velocity [m s$^{-1}$]")
...
def main() -> None:
    ...
    cache_dir = args.cache_dir or args.output_dir / "rms_vel1_time_pressure_cache"
    cache_dir.mkdir(parents=True, exist_ok=True)
    cache = cache_path(cache_dir, case_dir.name, args.first, args.last)
    if cache.exists() and not args.refresh_cache:
        times, pressures, rms_profiles, snapshots = read_cache(cache, case_dir.name)
        action = "Read"
    else:
        times, pressures, rms_profiles, snapshots = read_case_data(
            case_dir, args.first, args.last
        )
        write_cache(cache, case_dir.name, times, pressures, rms_profiles, snapshots)
        action = "Wrote"

    pressure_grid, rms_grid = interpolate_to_pressure_grid(
        pressures, rms_profiles, args.max_pressure
    )
    plot_times, plot_rms = filter_and_average_time(
        times,
        rms_grid,
        args.max_time,
    ...
    if args.running_average is not None:
        suffix += f"_running_average_{args.running_average:g}day"
    plot_path = args.output_dir / f"{case_dir.name}_rms_vel1_time_pressure_{suffix}.png"
    plot_contour(
        plot_path,
        plot_times,
        pressure_grid,
    ...
    print(
        f"Plotted {plot_times.size} snapshots through day {plot_times[-1]:g}"
        + (
            f" with a centered {args.running_average:g}-day running average"
            if args.running_average is not None
            else ""
        )
    )
